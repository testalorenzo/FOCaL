...
if __name__ == "__main__":
    ...
    plot = True  # Set to True to plot the results
    ...
    # Add noise
    Y0 += np.random.multivariate_normal(np.zeros(t), cov, n) # Add noise to control group
    Y1 += np.random.multivariate_normal(np.ones(t), cov, n) # Add noise to treatment group
    ...
    nuisance_mu = RandomForestRegressor(random_state=seed)
    nuisance_pi = RandomForestClassifier(random_state=seed)
    final_regression = RandomForestRegressor(random_state=seed)

    # Create FOCaL instance
    focal = FOCaL(nuisance_mu, nuisance_pi, final_regression)

    # Fit the model
    focal.fit(X, A, Y, n_folds=n_folds)
